...
const rgeocode = 'https://aginfo.cgk.affrc.go.jp/ws/rgeocode.php?json&lon=';
...
form3.addEventListener('submit', function (event) {
  ...
  if (r) {
    ...
    if (r) {
      ...
      r = s.match(/^北緯(\d+)度(\d+)分(\d+)秒, 東経(\d+)度(\d+)分(\d+)秒$/);
      if (r) {
        const lat = Number(r[1]) + (Number(r[2]) + Number(r[3]) / 60) / 60;
        const lon = Number(r[4]) + (Number(r[5]) + Number(r[6]) / 60) / 60;
